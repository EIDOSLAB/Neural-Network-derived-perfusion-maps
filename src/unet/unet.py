...
def conv_emb(in_channels, out_channels, kernel_size=3, stride=1,
          padding=1, bias=True, planar=False, dim=3):
    """Returns an appropriate spatial convolution layer, depending on args.
    - dim=2: Conv2d with 3x3 kernel
    - dim=3 and planar=False: Conv3d with 3x3x3 kernel
...
def conv_out(in_channels, out_channels, dim=3):
    """Returns a 89x1x1 convolution"""
...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pooling=True, planar=False, activation='relu',
                 normalization=None, full_norm=True, dim=3, conv_mode='same', embedding_time=False):
        super().__init__()
        ...
        if self.pooling:
            ...
            if planar:
                kernel_size = planar_kernel(kernel_size)
            self.pool = get_avgpool(dim)(kernel_size=kernel_size, ceil_mode=True) # NOTE: before it was get_maxpool() 
...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.att = None  # Field to store attention map for later analysis
...
class GridAttention(nn.Module):
    ...
    def __init__(self, in_channels, gating_channels, inter_channels=None, dim=3, sub_sample_factor=2):
        ...
        self.dim = dim
        self.sub_sample_kernel_size = (1, 2, 2) #self.sub_sample_factor
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_init = conv3(
                self.in_channels, self.out_channels, 
                kernel_size=(89, 1, 1), stride=(1, 1, 1),padding=(0, 0, 0), 
                planar=False, dim=dim
                )
        self.act_init = get_activation(activation)

        self.conv_final = conv1(outs, self.out_channels, dim=dim)
        #self.conv_final = conv_out(outs, self.out_channels, dim=dim)
