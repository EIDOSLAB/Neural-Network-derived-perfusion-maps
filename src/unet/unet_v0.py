...
def get_avgpool(dim=3):
    """Chooses an implementation for a avg-pooling layer."""
    if dim == 3:
        return nn.AvgPool3d
    elif dim == 2:
        return nn.AvgPool2d
...
def final_conv(in_channels, out_channels, dim=3, n_blocks=1):
    """Returns a tensor"""
    if n_blocks%2==0:
        filter_dim = 89 #int(89/(2**(n_blocks-1)))
    else:
        filter_dim = 89 #int(89/(2**(n_blocks-1)))+1
    return get_conv(dim)(
        in_channels, 
        out_channels, 
        kernel_size=(
...
def upconv_out(in_channels, out_channels, mode='transpose', planar=False, dim=3):
    """Returns a learned upsampling operator depending on args."""
    if 'linear' in mode:
        upsampling_mode = 'trilinear' if dim == 3 else 'bilinear'
    else:
    ...
    return ResizeConv(
        in_channels, out_channels, planar=planar, dim=dim,
        upsampling_mode=upsampling_mode, kernel_size=3, 
        scale_factor=(1, 2, 2)
    )


def downconv(in_channels, out_channels, mode='resizeconv', planar=False, dim=3, scale_factor=0.5):
    """Returns a learned upsampling operator depending on args."""
    kernel_size = 2
    stride = 2
    if planar:
...
def conv1(in_channels, out_channels, dim=3):
    ...
    return get_conv(dim)(in_channels, out_channels, kernel_size=(1, 1, 1))
...
def embedding_time(in_channels, out_channels, dim=3):
    """Returns a 1x1 or 1x1x1 convolution, depending on dim"""
    return get_conv(dim)(in_channels, out_channels, kernel_size=(89, 1, 1))
...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pooling=True, planar=False, activation='relu',
        ...
        if self.pooling:
            ...
            if planar:
                kernel_size = planar_kernel(kernel_size)
            self.pool = get_avgpool(dim)(kernel_size=kernel_size, ceil_mode=True)
...
class ResizeConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, planar=False, dim=3,
                 upsampling_mode='nearest', scale_factor=2):
        super().__init__()
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_blocks - 1): #-1
            ...
            up_conv = UpConv(
                ...
                planar=False, #prima era false; messo a true per non considerare la depth
                activation=activation,
                normalization=normalization,
                attention=attention,
                full_norm=full_norm,
            ...
            '''
            if i<1:
                up_conv.to(self.device[0])
            else:
                up_conv.to(self.device[2])
            '''
            self.up_convs.append(up_conv)

        #self.conv_final = conv1(outs, self.out_channels, dim=dim).to(self.device[2]) #self.out_channels

        
        self.downconv = downconv(
            self.in_channels, self.out_channels,
            mode='resizeconv_nearest', planar=True, dim=dim, 
            scale_factor=(1, 0.5, 0.5)
        ).to(self.device[0])


        self.resize = upconv_out(self.out_channels, 
            self.out_channels, 
            mode='resizeconv_nearest',
            planar=False,
            dim=dim
            ).to(self.device[1])

        self.conv_final = final_conv(
            outs, self.out_channels, dim=dim, n_blocks=self.n_blocks, 
        ).to(self.device[1])
    ...
    def forward(self, x):
        ...
        àx = self.downconv(x.to(self.device[0]))

        for module in self.down_convs:
            #if i<1:
                #x, before_pool = module(x)
            #else:
            module = module.to(self.device[0])
            x, before_pool = module(x.to(self.device[0])) 
        ...
        for module in self.up_convs:
            before_pool = encoder_outs[-(i+2)].to(self.device[1])
            x = module(before_pool, x.to(self.device[1]))
            i += 1


        # No softmax is used, so you need to apply it in the loss.
        #x = torch.sigmoid(self.conv_final(x))
        #x = self.conv_final(x.to(self.device[2]))
        #x = torch.sigmoid(x)
        x = self.conv_final(x.to(self.device[1]))
        #
        #x = self.resize(x)
        x = torch.sigmoid(x)
